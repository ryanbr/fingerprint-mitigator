...
2. **HTTP Client Hints** — `Sec-CH-UA*` headers, rewritten or stripped via `declarativeNetRequest` rules.
...
- `src/inject-identity.js` — `navigator.brave` deletion (walks prototype chain), brand filter on `userAgentData` (adds `Google Chrome` if missing — critical, this entry's absence is itself a Brave tell), vendor-globals strip, UA identity-token strip, customUA value override, `identity.uaDataMode = "remove"` deletes `navigator.userAgentData` entirely (Firefox/Safari presets).
...
- `src/background.js` — `registerInjectScripts()` registers each category script + the bridge with computed `excludeMatches`. `updateSecChUaRules()` builds a base DNR rule + per-site override rules. Refreshes on `chrome.storage.onChanged`. Default `FAKE_SEC_CH_UA` brand string; bump version when Chromium major rolls forward.
